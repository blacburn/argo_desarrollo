...
if($_REQUEST ['funcion'] == 'consultaContratoEstado'){
    ...
    $cadenaSql = $this->sql->getCadenaSql('buscar_contrato3', array('unidad' => $unidad
        , 'numero' => $_REQUEST ['numero'], 'vigencia_curso' => $_REQUEST ['vigencia']));
...
if ($_REQUEST ['funcion'] == 'consultaContrato') {
    ...
    $cadenaSqlUnidad = $this->sql->getCadenaSql("obtenerInfoUsuario", $_REQUEST['usuario']);
    $unidad = $DBFrameWork->ejecutarAcceso($cadenaSqlUnidad, "busqueda");
    $cadenaSql = $this->sql->getCadenaSql('buscar_contrato', array('parametro' => $_GET ['query'], 'unidad' => $unidad[0]['unidad_ejecutora']
