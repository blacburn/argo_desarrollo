...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------               
       ...
       // $ordenadorInfoExtra = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");
        ...
        if($_REQUEST['tipo_contrato'] =='Contrato de Prestación de Servicios Profesionales o Apoyo a la Gestión'){
            if ($nombreperfil == "ASISTENCIAL" || $nombreperfil == "NO APLICA" || $nombreperfil == "TÉCNICO") {
            $encabezadotipocontrato = " Y DE APOYO A LA GESTIÓN ADMINISTRATIVA DE LA ENTIDAD ";
            $tipo_contrato='Contrato de Prestación de Servicios de Apoyo a la Gestión';
            } else {
...
<tr>
        ...
        $cadenaSql = $this->miSql->getCadenaSql('consultaPlantilla', array('tipo_contrato' => $contrato['tipo_contrato'], 'tipo_plantilla' => 'plantillaJuridica',  'fecha_vigencia' => $contrato['fecha_registro']));
...
$mpdf = new mPDF('', 'LETTER', 10, 'ARIAL', 20, 15, 15, 15, 7, 10);
$mpdf->AddPage();
