...
//        
//        $cadenaSql = $this->miSql->getCadenaSql('ObtenerInfosupervisorAdicional', $supervisor[0]['documento']);
//        $supervisorAdicional = $DBSICA->ejecutarAcceso($cadenaSql, "busqueda");
//        
//        if($supervisorAdicional[0]['DIRECCION'] != null){
//             $supervisor_contrato.=" , Dirección: " .$supervisorAdicional[0]['DIRECCION'];
//            
//        }
//        if($supervisorAdicional[0]['TELEFONO'] != null ){
//            $supervisor_contrato.=" , Teléfono: " .$supervisorAdicional[0]['TELEFONO'];
//            
//        }
//        if($supervisorAdicional[0]['CORREO'] != null){
//           $supervisor_contrato.=" , Correo: " .$supervisorAdicional[0]['CORREO'];
//            
//        }
//        $supervisor_contrato.=" , Cédula de Ciudadanía: " .$supervisor [0]['documento'].". ";
        ...
        if ($contratista['tipopersona'] == 'NATURAL') {
            ...
            if ($representanteLegal[0]['cargo'] === 'REPRESENTANTE LEGAL' || $representanteLegal[0]['cargo'] === ' ') {
                ...
                $InfoContratistaContacto = '';
                if ($contratistaContacto =='') {
                    $InfoContratistaContacto = "Nombre: " . strtoupper($representanteLegal[0]['nombre'])  . ", Cargo: " . $representanteLegal[0]['cargo'] . ", Dirección: " . $contratista['direccion']  .", Teléfono: " . $contratista['numero_tel'] . ", Correo: " . $contratista['correo'];
                } else {
                    $InfoContratistaContacto = "Nombre: " . strtoupper($representanteLegal[0]['nombre'])  . ", Cargo: " . $representanteLegal[0]['cargo'] . ", Dirección: " . $contratista['direccion']  .", Teléfonos: " . $contratista['numero_tel'] . "-" . $contratistaContacto . ", Correo: " . $contratista['correo'];
                }
            }
        } else {
            $cadenaSql = $this->miSql->getCadenaSql('consultaRepresentanteLegal', $contrato ['contratista']);
...
<tr>
        ...
        $cadenaSql = $this->miSql->getCadenaSql('consultaPlantilla', array('tipo_contrato' => $contrato['tipo_contrato'], 'tipo_plantilla' => 'plantillaCompraVenta',  'fecha_vigencia' => $contrato['fecha_registro']));
        ...
        $parametros = array(
            ...
            'P[JUSTIFICACION]' => $contrato['justificacion'],
        ...
        $textos = array(0 => $contenidoPaginaEncabezado, 1 => $contenidoPagina, 2 => $contenidoPiePagina, 3 => $estilos, 4 => "Contrato Compra Venta " . $datosContrato[0] . '-' . $datosContrato[1]);
