...
class Sql extends \Sql {
    ...
    function getCadenaSql($tipo, $variable = "") {
        ...
        switch ($tipo) {
            ...
            case "registrarOrdenadorApp" :
                $cadenaSql = " INSERT INTO argo.ordenadores (";
                $cadenaSql .= " id_ordenador, info_resolucion, id_ciudad, fecha_inicio, fecha_fin, 
                                estado, documento, nombre_ordenador, rol_ordenador, rol_id) ";
                $cadenaSql .= " VALUES (";
                $cadenaSql .= "'" . $variable ['id_ordenador'] . "',";
                $cadenaSql .= "'" . $variable ['resolucion'] . "',";
                $cadenaSql .= "'" . $variable ['ciudadExp'] . "',";
            ...
            case "rolesOrdenadores" :

               $cadenaSql = " SELECT id  as data, rol  as value ";
               $cadenaSql .= " FROM argo.rol_ordenador ";
               $cadenaSql .= " ORDER BY value; ";
                break;

            case "cargosOrdenadorById" :
            ...
            case "consultarMayorIdOrd" :

                $cadenaSql = "SELECT max(id)  ";
                $cadenaSql .= "FROM argo.ordenadores sc ";
                $cadenaSql .= "WHERE 1=1; ";
                break;

            case "consultarOrdenadorGeneral" :
            ...
            case "actualizarSupervisorApp" :
                $cadenaSql = " UPDATE argo.supervisor_contrato SET ";
                $cadenaSql .= "nombre = '" . $variable ['nombre'] . "', ";
                $cadenaSql .= "documento = '" . $variable ['documento'] . "', ";
                $cadenaSql .= "digito_verificacion = '" . $variable ['digito'] . "', ";
                if($variable ['fecha_inicio'] != NULL){
                    $cadenaSql .= "fecha_inicio = '" . $variable ['fecha_inicio'] . "', ";
                }
                if($variable ['fecha_fin'] != NULL){
            ...
            case "actualizarOrdenadorApp" :
                $cadenaSql = " UPDATE argo.ordenadores SET ";
                $cadenaSql .= "nombre_ordenador = '" . $variable ['nombre'] . "', ";
                $cadenaSql .= "documento = '" . $variable ['documento'] . "', ";
                $cadenaSql .= "id_ordenador = '" . $variable ['id_ordenador'] . "', ";
                if($variable ['fecha_inicio'] != NULL){
                    $cadenaSql .= "fecha_inicio = '" . $variable ['fecha_inicio'] . "', ";
                }
                if($variable ['fecha_fin'] != NULL){
                ...
                $cadenaSql .= "id_ciudad = '" . $variable ['ciudadExp'] . "', ";
                $cadenaSql .= "info_resolucion = '" . $variable ['resolucion'] . "'";
                $cadenaSql .= " WHERE id = '".$variable ['id']."';";
                break;    

            case "recuperarDatosOrdenador" :

                $cadenaSql = " SELECT * ";
                $cadenaSql .= " FROM argo.ordenadores ";
                $cadenaSql .= " WHERE id = ".$variable."; ";
                break;

            case "recuperarDatosSupervisor" :

                $cadenaSql = " SELECT * ";
                $cadenaSql .= " FROM argo.supervisor_contrato ";
                $cadenaSql .= " WHERE id = ".$variable."; ";
                break;

            case "registrarSupervisorApp" :
                $cadenaSql = " INSERT INTO argo.supervisor_contrato (";
                $cadenaSql .= " nombre, documento, cargo, sede_supervisor, dependencia_supervisor, 
                                tipo, estado, digito_verificacion, fecha_inicio, fecha_fin, cargo_id) ";
                $cadenaSql .= " VALUES (";
                $cadenaSql .= "'" . $variable ['nombre'] . "',";
                $cadenaSql .= "'" . $variable ['documento'] . "',";
                ...
                $cadenaSql .= "'" . $variable ['fecha_inicio'] . "',";
                $cadenaSql .= "'" . $variable ['fecha_fin'] . "',";
                $cadenaSql .= "'" . $variable ['cargo_id'] . "'";
                $cadenaSql .= " );";
                break;

            case "cargosFuncionarioById" :

               $cadenaSql = " SELECT id  as data, cargo  as value ";
               $cadenaSql .= " FROM argo.cargo_supervisor_temporal ";
               $cadenaSql .= " WHERE id = ".$variable."; ";
                break;

            case "rolesFuncionarioById" :
            ...
            case "sedeSup" :
                ...
                $cadenaSql .= 'FROM argo."sedes_SIC" ';
                $cadenaSql .= 'WHERE "ESF_ID_SEDE" = '."'".$variable."' LIMIT 1; ";
                break;
            ...
            case "dependenciaSup" :
                ...
                $cadenaSql .= 'WHERE "ESF_CODIGO_DEP" = '."'".$variable."' LIMIT 1; ";
                break;
            ...
            case "consultaSupervisoresCom" :

                $cadenaSql = "SELECT * ";
                $cadenaSql .= "FROM argo.supervisor_contrato sc ";
                $cadenaSql .= "WHERE 1=1 ";
                if ($variable ['sede'] != '') {
                    $cadenaSql .= " AND sc.sede_supervisor = '" . $variable ['sede'] . "' ";
                }
                if ($variable ['dependencia'] != '') {
                    $cadenaSql .= " AND sc.dependencia_supervisor = '" . $variable ['dependencia'] . "' ";
                }
                if ($variable ['cargo'] != '') {
                    $cadenaSql .= " AND sc.cargo_id = '" . $variable ['cargo'] . "' ";
                }
                $cadenaSql .= "  ORDER BY sc.fecha_fin DESC LIMIT 5; ";
                break;

            case "consultaOrdenadoresCom" :

                $cadenaSql = "SELECT * ";
                $cadenaSql .= "FROM argo.ordenadores sc ";
                $cadenaSql .= "WHERE 1=1 ";
                if ($variable ['rol'] != '') {
                    $cadenaSql .= " AND sc.rol_id = '" . $variable ['rol'] . "' ";
                }
                $cadenaSql .= "  ORDER BY sc.fecha_fin DESC LIMIT 5; ";
                break;    

            case "consultarSupervisorGeneral" :
            ...
            case "ordenadorGasto" :
                ...
                $cadenaSql = " SELECT ORG_IDENTIFICACION ||'-'|| ORG_NOMBRE ||'-'|| ORG_IDENTIFICADOR as data, '('|| ORG_IDENTIFICADOR ||') '|| ORG_NOMBRE as value FROM SICAARKA.ORDENADORES_GASTO ";
