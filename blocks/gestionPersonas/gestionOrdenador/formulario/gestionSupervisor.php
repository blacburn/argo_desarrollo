...
class registrarForm {
    ...
    function miForm() {
        ...
        //******************************************************************************************************
        //************************************** DATOS SUPERVISOR **********************************************
        $cadenaSql = $this->miSql->getCadenaSql('recuperarDatosSupervisor', $_REQUEST ['id']);
        $datosSupervisor = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");

        if(isset($datosSupervisor[0]['tipo'])){ $_REQUEST['tipo_supervisor'] = $datosSupervisor[0]['tipo'];}
        if(isset($datosSupervisor[0]['nombre'])){ $_REQUEST['nombre_supervisor'] = $datosSupervisor[0]['nombre'];}
        if(isset($datosSupervisor[0]['documento'])){ $_REQUEST['documento_supervisor'] = $datosSupervisor[0]['documento'];}
        if(isset($datosSupervisor[0]['digito_verificacion'])){ $_REQUEST['digito_supervisor'] = $datosSupervisor[0]['digito_verificacion'];}

        if(isset($datosSupervisor[0]['fecha_inicio'])){ $_REQUEST['fecha_inicio_mod'] = $this->cambiafecha_format($datosSupervisor[0]['fecha_inicio']);}
        if(isset($datosSupervisor[0]['fecha_fin'])){ $_REQUEST['fecha_fin_mod'] = $this->cambiafecha_format($datosSupervisor[0]['fecha_fin']);}

        if(isset($datosSupervisor[0]['fecha_inicio'])){ $_REQUEST['fecha_inicio_hidden'] = $datosSupervisor[0]['fecha_inicio'];}
        if(isset($datosSupervisor[0]['fecha_fin'])){ $_REQUEST['fecha_fin_hidden'] = $datosSupervisor[0]['fecha_fin'];}

        if(isset($datosSupervisor[0]['cargo_id'])){ $_REQUEST['cargo_hidden'] = $datosSupervisor[0]['cargo_id'];}
        //******************************************************************************************************
        //******************************************************************************************************
        ...
        $atributos ['tipoEtiqueta'] = 'inicio';
        $atributos ["leyenda"] = "Gestionar Supervisor";
        echo $this->miFormulario->marcoAgrupacion('inicio', $atributos); {
            

            $tipo = 'information';
            $mensaje =  "<br>Gestión <b>Supervisor</b>
        ...
        $valorCodificado .= "&opcion=modificarSupervisor";
