...
//--------------Fin JavaScript y Convenios x Vigenca --------------------------------------------------------------------------------------------------  
    ...
    $("#<?php echo $this->campoSeguro('tipo_supervisor') ?>").change(function () {
        if ($("#<?php echo $this->campoSeguro('tipo_supervisor') ?>").val() == 1) {
            ...
            $("#<?php echo $this->campoSeguro('documento_supervisor') ?>").val('');
            $("#<?php echo $this->campoSeguro('digito_supervisor') ?>").val('');

        } else if($("#<?php echo $this->campoSeguro('tipo_supervisor') ?>").val() == 2) {
    ...
    $("#<?php echo $this->campoSeguro('funcionario') ?>").change(function () {
        ...
        if ($("#<?php echo $this->campoSeguro('funcionario') ?>").val() != '') {
            ...
            $("#<?php echo $this->campoSeguro('nombre_supervisor') ?>").val(data[1]);
            $("#<?php echo $this->campoSeguro('documento_supervisor') ?>").val(data[0]);
    ...
    $("#<?php echo $this->campoSeguro('cargo') ?>").change(function () {
        if ($("#<?php echo $this->campoSeguro('cargo') ?>").val() != '') {
            consultarCombinacionCargo();
            var cargoOpt = $("#<?php echo $this->campoSeguro('cargo') ?> option:selected").text();
            $("#<?php echo $this->campoSeguro('cargo_hidden') ?>").val(cargoOpt);
        }
    ...
    function consultarCombinacionCargo(elem, request, response) {
        $.ajax({
            url: "<?php echo $urlFinalComCargo ?>",
            dataType: "json",
            data: {cargo: $("#<?php echo $this->campoSeguro('cargo') ?>").val(), 
                    sede: $("#<?php echo $this->campoSeguro('sede') ?>").val(), 
                    dependencia: $("#<?php echo $this->campoSeguro('dependencia') ?>").val()},
            success: function (data) {
                ...
                if (data) {
                    ...
                    if(dtFin >= actual && dtIni <= actual){
                        ...
                        var sedeOpt = $("#<?php echo $this->campoSeguro('sede') ?> option:selected").text();
                        var dependenciaOpt = $("#<?php echo $this->campoSeguro('dependencia') ?> option:selected").text();
                        var cargoOpt = $("#<?php echo $this->campoSeguro('cargo') ?> option:selected").text();
                        ...
                        swal({
                            ...
                            html:
                                    ...
                                    +'Cargo: <b>'+cargoOpt+'</b><br>'
                                    +'<br>'
                                    +'Existe supervisor <b>ACTIVO</b>:<br>'
                                    +'<pre>('+data[0]['documento']+') '+ data[0]['nombre']+'</pre><br>'
                                    +'No se puede registrar este cargo hasta vencido el mismo en, <b>FECHA DE FIN</b>  (<b>'+formatDateAdd(data[0]['fecha_fin'], 2)+')</b>, por favor tenga en cuenta esto.',
                            confirmButtonText:
                                    'Aceptar'
                        })
                        ...
                        swal({
                            ...
                            type: 'info',
                            html:
                                    'Tenga en cuenta que, actualmente los últimos supervisores registrados para:<br><br>'
                                    +'Sede: <b>'+sedeOpt+'</b><br>'
                                    +'Dependencia: <b>'+dependenciaOpt+'</b><br>'
                                    +'Cargo: <b>'+cargoOpt+'</b><br><br>son,<br><br>'
                                    +tablaHt
                                    +'<br>'
...
function calcularDigitoCedula(cadenaCedula){
    ...
    if(isNaN(cadenaCedula)){ 
        $("#<?php echo $this->campoSeguro('digito_supervisor')?>").val(null);
    }
    else{
