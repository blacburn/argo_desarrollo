...
namespace gestionActasInicio;

use gestionActasInicio\funcion\redireccion;
        ...
        switch ($tipo) {
            ...
            case "consultarElementosContratista" :
                ...
                $cadenaSql .= " total_iva, total_iva_con, elemento.descripcion,
								sas.\"ESF_SEDE\" sede, ad.\"ESF_DEP_ENCARGADA\" dependencia,espacios.\"ESF_NOMBRE_ESPACIO\" espacio_fisico, tb.descripcion tipo_bien	";
                ...
                $cadenaSql .= ' JOIN arka_inventarios.tipo_bienes as tb ON tb.id_tipo_bienes=elemento.tipo_bien ';
...
//                
            ...
            case "consultarContratosGeneralUE" :
                ...
                $cadenaSql .= "AND cg.numero_contrato = ce.numero_contrato and cg.vigencia = ce.vigencia and ce.estado = ec.id ";
                $cadenaSql .= "AND ce.fecha_registro = (SELECT MAX(cee.fecha_registro) from contrato_estado cee where cg.numero_contrato = cee.numero_contrato and  cg.vigencia = cee.vigencia) ";
                $cadenaSql .= "and tpc.id_grupo_tipo_contrato = 2 ";
            ...
            case "consultarContratosGeneral" :
                ...
                $cadenaSql = "SELECT DISTINCT cg.clase_contratista,p.descripcion, cg.numero_contrato, "
                        . " cg.vigencia, cg.fecha_registro, cg.contratista as proveedor,cg.tipologia_contrato, "
                        . " ec.nombre_estado, tpc.tipo_contrato,"
                        . " ce.fecha_registro as fecha_registro_estado, cg.convenio, cs.numero_contrato_suscrito , cs.fecha_suscripcion, cg.unidad_ejecutora ";
                $cadenaSql .= "FROM parametros p, contrato_general cg, tipo_contrato tpc , ";
                $cadenaSql .= "contrato_estado ce, estado_contrato ec, contrato_suscrito cs  ";
                $cadenaSql .= "WHERE cg.tipologia_contrato = p.id_parametro AND  cg.tipo_contrato = tpc.id ";
                $cadenaSql .= "AND cg.numero_contrato = cs.numero_contrato and cg.vigencia = cs.vigencia ";
            ...
            case "obtenerInfoUsuario" :
                $cadenaSql = "SELECT u.dependencia_especifica ||' - '|| u.dependencia as nombre, unidad_ejecutora , identificacion ";
            ...
            case "consultarCargoSupervisor" :
                ...
                $cadenaSql .= " JOIN argo.contrato_general as cg ON cg.supervisor=sc.id  ";
                $cadenaSql .= " WHERE numero_contrato='" . $variable['numero_contrato'] . "'  AND vigencia=" . $variable['vigencia'] . "; ";
            ...
            case 'consultaLugarEjecucion' :
                $cadenaSql = " SELECT id, direccion, sede, dependencia, ciudad , c.nombre nombre_ciudad ";
                $cadenaSql .= " FROM argo.lugar_ejecucion ";
                $cadenaSql .= " JOIN core.ciudad as c ON c.id_ciudad= lugar_ejecucion.ciudad";
