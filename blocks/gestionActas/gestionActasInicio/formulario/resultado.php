...
class registrarForm {
    ...
    function miForm() {
        ...
        //----------------------------------------------------------------------------------------------- CONTRATO -------------------------
        //**********************************************************************************************************************************
        if(isset($_REQUEST['tipo_acceso']) && $_REQUEST['tipo_acceso'] == 'contrato'){
                ...
                if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                    ...
                    $unidadEjecutora = $esteRecursoDB->ejecutarAcceso($cadenaSqlUnidad, "busqueda");
            ...
            if ($contratos) {
                $mensaje = " El documento generado para las actas de inicio sigue el formato CPS de actas de inicio del SIGUD.<br>";
                $mensaje .= " Si aplica, puede hacer uso del formato mencionado anteriormente. En caso de que dicho documento no aplique, debe hacer el formato manualmente.";


                // ---------------- CONTROL: Cuadro de Texto --------------------------------------------------------
                $esteCampo = 'mensajeDocumento';
                $atributos ['id'] = $esteCampo;
                ...
                foreach ($contratos as $valor) {
                    ...
                    if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                        $variable = "pagina=consultaContratosAprobadosC"; // pendiente la pagina para modificar parametro
                    } else {
                    ...
                    // SI EXISTE CONTRATO TIPO SUSCRITO


                    if ($valor['nombre_estado'] == 'Suscrito') {
                        ...
                        //SI EXISTE FECHA FIN EN TABLA ACTA DE INICIO


                        if ($fecha_fin[0]['fecha_fin']) {

                            //  SI EL CONRATO NO TIENE RPS
                            ...
                            if (!$resultadoRps) {
                                ...
                                $variable_acta_inicio .= "&tipo_acceso=" . $_REQUEST['tipo_acceso'];
                                if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                            ...
                            // SE CALCULA FECHA FIN Y SE VERIFICA SI SI ES MAYOR O IGUAL A HOY Y SI EL CONTRATO TIENE RPS
                        ...
                        $variable_mod_acta_inicio .= "&tipo_acceso=" . $_REQUEST['tipo_acceso'];
                        if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {

                            $variable_mod_acta_inicio .= "&accesoCondor=true";
                        }
                        ...
                        $mod_acta_inicio = "<a href='" . $variable_mod_acta_inicio . "'><img src='" . $rutaBloque . "/css/images/editar.png' width='15px'></a>";
                    ...
                    if ($validacion_doc_act == 1) {
                        ...
                        $variable_documento .= "&opcion=generarDocumento";
                        $variable_documento .= "&tipo_acceso=" . $_REQUEST['tipo_acceso'];
                        if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
    ...
    //-------------------------- FIN CONTROLES PARA TABLA
        ...
        //----------------------------------------------------------------------------------------------- NOVEDADES ------------------------
        //**********************************************************************************************************************************
        }else if(isset($_REQUEST['tipo_acceso']) && $_REQUEST['tipo_acceso'] == 'novedad'){
            ...
            $conexion = "contractual";
                ...
                if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                    ...
                    $arreglo = array(
                        ...
                        'fecha_inicial' => $fecha_inicio,
                        'fecha_final' => $fecha_final,
                        'vigencia_curso' => $_REQUEST['vigencia_por_contrato_nov']
                    );
               ...
               $ids_dep = substr($ids_dep, 0, -1);
                ...
                if ($id_supervisor) {
                    ...
                    $arreglo = array(
                        'clase_contrato' => $clase_contrato,
                        'numero_contrato' => $contrato,
                        'vigencia' => $_REQUEST['vigencia_por_contrato_nov'],
                        'nit' => $contratista,
                        'fecha_inicial' => $fecha_inicio,
                        'fecha_final' => $fecha_final,
                        'unidad_ejecutora' => $unidad_ejecutora_consulta,
                    ...
                    $cadenaSql = $this->miSql->getCadenaSql('consultarContratosNovedadesPost', $arreglo);
                    $contratos = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");
            ...
            $atributos ["estilo"] = "jqueryui";
            $atributos ['tipoEtiqueta'] = 'inicio';
            $atributos ["leyenda"] = "Novedades a Contratos - Acta de Inicio";
            echo $this->miFormulario->marcoAgrupacion('inicio', $atributos);
            ...
            if ($contratos) {
                ...
                echo "<table id='tablaContratosAprobadosNov'>";
                ...
                foreach ($contratos as $valor) {
                    ...
                    $variable .= "&opcion=consultarContrato";
                    $variable .= "&consultaSupervisor=TRUE";
                    $variable .= "&numero_contrato=" . $valor ['numero_cons'];
                    $variable .= "&numero_contrato_suscrito=" . $valor ['numero_contrato_suscrito'];
                    $variable .= "&vigencia=" . $valor ['vigencia'];
                    $variable .= "&mensaje_titulo= --> Contrato: " . $valor ['tipo_contrato'] . " | VIGENCIA: " . $valor ['vigencia'] . " | NÚMERO CONTRATO : " . $valor ['numero_contrato_suscrito'];
                    $variable .= "&arreglo=" . $arreglo;
                    ...
                    if($valor['contratista'] > 0){
                        $tipo_nov = "Cesión";
                        $tipo_novP = 1;
                        $valor['proveedor'] = $valor['contratista'];
                    }else{
                        $tipo_nov = "Otrosi";
                        $tipo_novP = 2;
                        $valor['proveedor'] = $valor['contratista_cg'];
                    }
                    ...
                    $cadenaSql = $this->miSql->getCadenaSql('consultarActasxNovedadPost', $valor['id']);
                    $novedad = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");

                    $fecha_inicio_acta = $novedad[0]['fecha_inicio'];
                    $fecha_fin_acta = $novedad[0]['fecha_fin'];

                    if (!$novedad[0]['fecha_fin']) {


                        $variable_acta_inicio = "&pagina=" . $this->miConfigurador->getVariableConfiguracion('pagina');
                        $variable_acta_inicio .= "&opcion=actainicioNov";
                        $variable_acta_inicio .= "&idNov=" . $valor ['id'];
                        $variable_acta_inicio .= "&vigencia=" . $valor ['vigencia'];
                        $variable_acta_inicio .= "&contratista=" . $valor ['proveedor'];
                        $variable_acta_inicio .= "&arreglo=" . $arreglo;
                        $variable_acta_inicio .= "&tipo_acceso=" . $_REQUEST['tipo_acceso'];
                        if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                            $variable_acta_inicio .= "&accesoCondor=true";
                        }
                        $variable_acta_inicio .= "&numero_contrato=" . $valor ['numero_cons'];
                        $variable_acta_inicio .= "&numero_contrato_suscrito=" . $valor ['numero_contrato_suscrito'];
                        $variable_acta_inicio .= "&usuario=" . $_REQUEST ['usuario'];
                        $variable_acta_inicio .= "&mensaje_titulo= --> Contrato: " . $valor ['tipo_contrato'] . " | VIGENCIA: " . $valor ['vigencia'] . " | NÚMERO CONTRATO : " . $valor ['numero_contrato_suscrito'];
                        $variable_acta_inicio = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($variable_acta_inicio, $directorio);
                        ...
                        //$cadenaSql = $this->miSql->getCadenaSql('consultarFechaFinActa2', $novedad[0]['id']);
                        //$fecha_fin_acta = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");


                        $variable_mod_acta_inicio = "&pagina=" . $this->miConfigurador->getVariableConfiguracion('pagina');
                        $variable_mod_acta_inicio .= "&opcion=modactainicioNov";
                        $variable_mod_acta_inicio .= "&idNov=" . $valor ['id'];
                        $variable_mod_acta_inicio .= "&vigencia=" . $valor ['vigencia'];
                        $variable_mod_acta_inicio .= "&contratista=" . $valor ['proveedor'];
                        $variable_mod_acta_inicio .= "&arreglo=" . $arreglo;
                        $variable_mod_acta_inicio .= "&tipo_acceso=" . $_REQUEST['tipo_acceso'];
                        if (isset($_REQUEST ['accesoCondor']) && $_REQUEST ['accesoCondor'] == 'true') {
                        ...
                        $variable_mod_acta_inicio .= "&numero_contrato=" . $valor ['numero_cons'];
                        $variable_mod_acta_inicio .= "&numero_contrato_suscrito=" . $valor ['numero_contrato_suscrito'];
                        $variable_mod_acta_inicio .= "&usuario=" . $_REQUEST ['usuario'];
                        $variable_mod_acta_inicio .= "&mensaje_titulo= --> Contrato: " . $valor ['tipo_contrato'] . " | VIGENCIA: " . $valor ['vigencia'] . " | NÚMERO CONTRATO : " . $valor ['numero_contrato_suscrito'];
                        $variable_mod_acta_inicio = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($variable_mod_acta_inicio, $directorio);
                    ...
                    //******************************* DOCUMENTO ACTA DE INICIO - NOVEDAD ***************************************
                    ...
                    if ($validacion_doc_act == 1) {
                        ...
                        $variable_documento .= "&bloqueGrupo=gestionContractual/contratos/";
                        $variable_documento .= "&opcion=generarDocumento";
                        $variable_documento .= "&numero_contrato=" . $valor ['numero_cons'];
                        $variable_documento .= "&numero_contrato_suscrito=" . $valor ['numero_contrato_suscrito'];
                        $variable_documento .= "&vigencia=" . $valor ['vigencia'];
                        $variable_documento .= "&usuario=" . $_REQUEST['usuario'];
                        $variable_documento .= "&tipo_contrato=" . $valor ['tipo_contrato'];
...
//                                   $datosenvio="'". $valor ['vigencia'] . $valor ['numero_cons'] ."&".$tipo_nov."'";
                        $documentoNov = "<a href='javascript:void(0);' onclick='GenerarDocumentoNov(" . $valor ['vigencia'] . $novedad[0]['id']. ");'><img src='" . $rutaBloque . "/css/images/documento.png' width='15px'> </a>";
                    } else {
                        $documentoNov = "<a><img src='" . $rutaBloque . "/css/images/inhabilitado.png' width='15px'></a>"; 
                    }
                    //******************************* DOCUMENTO ACTA DE INICIO - NOVEDAD ***************************************
                    ...
                    $mostrarHtml .="<td><center>" . $fecha_fin_acta . "</center> </td>
                                   <td><center>" . $documentoNov . "</center> </td>
                             </tr>";
                    echo $mostrarHtml;
                    unset($mostrarHtml);
                    unset($variable);
