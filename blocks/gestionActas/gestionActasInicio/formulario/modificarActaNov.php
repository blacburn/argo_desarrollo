...
//        }
        ...
        $atributos ['tipoEtiqueta'] = 'inicio';
        $atributos ["leyenda"] = "Modificar Acta de Inicio Novedad: " . $_REQUEST['mensaje_titulo'];
...
//--------- Pero como aun no se implmenetan las polizas a nivel contractual se deja comentado para su posterior ajuste
        ...
        $atributos ['marco'] = true;
        $atributos ["etiqueta"] = "";
        $atributos ['valor'] = $_REQUEST['idNov'];
        $atributos = array_merge($atributos, $atributosGlobales);
        echo $this->miFormulario->campoCuadroTexto($atributos);

        $esteCampo = "marcoDatosBasicosAc";
        $atributos ['id'] = $esteCampo;
        $atributos ["estilo"] = "jqueryui";
        $atributos ['tipoEtiqueta'] = 'inicio';
        $atributos ["leyenda"] = "Datos de Actas de Inicio";
        echo $this->miFormulario->marcoAgrupacion('inicio', $atributos);
    ...
    <div class='row'>
        <div class='col-md-18'>
            ...
            */
            ...
            // INFORMACION INVENTARIOS CONTRATISTA
            ...
            $cadenaSql = $this->miSql->getCadenaSql('Consultar_Novedad_Particular', $_REQUEST['idNov']);
            $novedad = $DBContractual->ejecutarAcceso($cadenaSql, "busqueda");


            $cadenaSql = $this->miSql->getCadenaSql('ConsultarDescripcionParametro', $novedad[0]['unidad_ejecucion']);
            $plazo_ejecion_consulta = $DBContractual->ejecutarAcceso($cadenaSql, "busqueda");
            $plazo_ejecucion = $novedad[0]['plazo_ejecucion'] . " " . $plazo_ejecion_consulta[0][0];

/*
            $cadenaSql = $this->miSql->getCadenaSql('obtenerInfoProveedor', $_REQUEST ['contratista']);
            ...
            //$cadenaSql = $this->miSql->getCadenaSql('consultarActasxContratista', $contrato[0]['contratista']);
            //$fecha_fin = $DBContractual->ejecutarAcceso($cadenaSql, "busqueda");
*/

            if($novedad[0]['tipo_novedad'] === '220')
            {

                 $datosConsultacesion = array(
                    'numero_contrato' => $_REQUEST['numero_contrato_suscrito'],
                    'vigencia' => $_REQUEST ['vigencia']
                 );

                     
                $cadenaSql = $this->miSql->getCadenaSql('consultarNovedadCesion', $datosConsultacesion);
                $novedadCesion = $DBContractual->ejecutarAcceso($cadenaSql, "busqueda");

                if($novedadCesion){
                     $cadenaSql = $this->miSql->getCadenaSql('obtenerInfoProveedor', $novedadCesion [0]['contratista']);
                     $contratista = $esteRecursoDBAgora->ejecutarAcceso($cadenaSql, "busqueda");
            ...
            $cadenaSql = $this->miSql->getCadenaSql('consultarActasxNovedadPost', $_REQUEST ['idNov']);
            $novedad = $DBContractual->ejecutarAcceso($cadenaSql, "busqueda");

            $fechaFinVal = $novedad[0]['fecha_fin'];
            $fechaIniVal = $novedad[0]['fecha_inicio'];
            ...
            if ($contrato[0]['tipo_contrato'] === '6' || $contrato[0]['tipo_contrato'] === '17') {
                if ($novedad[0]['fecha_fin']) {
                    ...
                    $cadenaSql = $this->miSql->getCadenaSql('consultarFechaFinActa2Nov', $novedad[0]['id']);
            ...
            */
            ...
            $_REQUEST ['fecha_inicio_acta'] = $fechaIniVal;
            $_REQUEST ['fecha_final_acta'] = $fechaFinVal;
            $mensajeVal = 'mensaje';
            ...
            if ($mensajeVal == 'mensajeInfo') {
                ...
                $mensaje = "INFORMACIÓN DEL CONTRATISTA. <br><br>" . "Se Realizará Registro de Acta de Inicio al contratista: <br><br>";
                $mensaje .= " Nombre del Contratista : " . $contratista[0]['nom_proveedor'] . "<br>";
                $mensaje .= "Tipo Documento: " . $tipoDocumento[0][0] . " <br>";
                $mensaje .= "Documento: " . $contratista[0]['num_documento'] . " <br>";
                $mensaje .= "Tipo Persona: " . $contratista[0]['tipopersona'] . " <br><br>";
...
/*        } else {
        ...
        $valorCodificado .= "&opcion=modificarActaInicioNov";
        $valorCodificado .= "&id_acta=" . $_REQUEST['idNov'];
