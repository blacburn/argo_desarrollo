...
//----------------------------------------------------------------------------------------------------------------------------------------------------------------               
        ...
        $datosNovedad = array(
            0 => $_REQUEST ['idNovedad'],
            1 => $_REQUEST ['vigencia'],
            'id_Novedad' => $_REQUEST ['idNovedad'],
            'vigencia' => $_REQUEST ['vigencia']
        );


        // Obtiene informacion del contrato para busquedas a partir de la novedad

        $cadenaSql = $this->miSql->getCadenaSql('consultarContratosxNovedadPost', $datosNovedad['id_Novedad']);
      
        $info_contrato_gral = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");
     
        
        
        // Obtiene Información de cargo del supervisor del contrato

        $datosContrato = array(
            0 => $info_contrato_gral[0]['numero_contrato'],
            1 => $info_contrato_gral[0]['vigencia'],
            'numero_contrato' => $info_contrato_gral[0]['numero_contrato'],
            'vigencia' => $info_contrato_gral[0]['vigencia']
        );
        ...
        $cadenaSql = $this->miSql->getCadenaSql('consultarActasInicioNovedad', $datosNovedad['id_Novedad']);
...
//        $polizas = $esteRecursoDB->ejecutarAcceso($cadenaSql, "busqueda");
        ...
        for ($i = 0; $i < count($actas_inicio); $i++) {
            ...
            $numero_poliza = $numero_poliza . $actas_inicio[$i]['numero_poliza'] . "<br>";
            $aseguradoras = $aseguradoras . $actas_inicio[$i]['nombre_aseguradora'] . "<br>";
        }
        ...
        if ($actas_inicio[0]['tipo_novedad'] == 220) {
            $variableTituloContrato = "OTROSÍ AL CONTRATO No. ";
        ...
        if ($actas_inicio[0]['tipo_novedad'] == 219) {
            
            $variableTituloContrato = "CONTRATO No. ";

            $cadenaSql = $this->miSql->getCadenaSql('obtenerInfoProveedor', $actas_inicio[0]['contratista']);
            $contratista = $esteRecursoDBAgora->ejecutarAcceso($cadenaSql, "busqueda");
        }
        ...
        if ($actas_inicio[0]['tipo_novedad'] == 220) {
            $Letras = $funcionLetras->ValorEnLetras($actas_inicio[0]['valor_novedad'], ' Pesos ');
            $valorContrato = $Letras . "($" . number_format($actas_inicio[0]['valor_novedad'], 2, ",", ".") . ")";
        }
        if ($actas_inicio[0]['tipo_novedad'] == 219) {
            
            
            if($contrato['unidad_ejecucion']==205){
                $valor_diario= $contrato['valor_contrato']/($contrato['plazo_ejecucion']);
            }
            if($contrato['unidad_ejecucion']==206){
                $valor_diario= $contrato['valor_contrato']/($contrato['plazo_ejecucion']*30);
            }
            if($contrato['unidad_ejecucion']==207){
                $valor_diario= $contrato['valor_contrato']/($contrato['plazo_ejecucion']*360);
            }
            
            
            if($actas_inicio[0]['unidad_ejecucion']==205){
                $unidad_ejecucion_acta= ($actas_inicio[0]['plazo_ejecucion']);
            }
            if($actas_inicio[0]['unidad_ejecucion']==206){
                $unidad_ejecucion_acta= ($actas_inicio[0]['plazo_ejecucion']*30);
            }
            if($actas_inicio[0]['unidad_ejecucion']==207){
                $unidad_ejecucion_acta= ($actas_inicio[0]['plazo_ejecucion']*360);
            }
   
            $valor_cesion= $valor_diario*$unidad_ejecucion_acta;
            
            $Letras = $funcionLetras->ValorEnLetras($valor_cesion, ' Pesos ');
            $valorContrato = $Letras . "($" . number_format($valor_cesion, 2, ",", ".") . ")";
        }
        ...
        if ($actas_inicio [0]['unidad_ejecucion'] == '205') {
            $meses = $actas_inicio [0]['plazo_ejecucion'] / 30;
            if ($meses > 1) {
                $meses = floor($meses);
                $parcial = $meses * 30;
                $dias = $actas_inicio [0]['plazo_ejecucion'] - $parcial;
                $plazo = $meses . " mes(es) y " . $dias . " dia(s) ";
            } else {
                $plazo = $actas_inicio [0]['plazo_ejecucion'] . " dia(s) ";
            }
        } elseif ($actas_inicio [0] ['unidad_ejecucion'] == '206') {
            $plazo = $actas_inicio [0]['plazo_ejecucion'] . " Mes(es) ";
        } else {
            $meses = $actas_inicio [0]['plazo_ejecucion'] * 12;
            $plazo = $meses . " mes(es) ";
        }
        ...
        $fechaAprobacionpoliza = explode("-", $actas_inicio[0]['fecha_aprobacion']);
        setlocale(LC_TIME, "es_ES.UTF-8");
        ...
        $fechaExpedicionpoliza = explode("-", $actas_inicio[0]['fecha_expedicion']);
        setlocale(LC_TIME, "es_ES.UTF-8");
...
<page backtop='5mm' backbottom='5mm' backleft='10mm' backright='10mm'>
        ...
        $contenidoPagina .= "
                ...
                <table align='justify' style='width:100%;' >                   
                     <tr>
                        <td style='width:5%;text-align:left;height: 30px'></td>	
                        <td style='width:35%;text-align:justify;height: 30px'><font size='3px'><b>".$variableTituloContrato."</b></font></td>	
                        <td style='width:55%;text-align:justify;height: 30px'><font size='3px'><b>" . $info_contrato_gral[0]['numero_contrato_suscrito'] . '</b>  suscrito el ' . $fechaSucripcion . "</font></td>	
                        <td style='width:5%;text-align:left;height: 30px'></td>	 
                    </tr>
                    <tr>
                        <td style='width:5%;text-align:left;height: 30px'></td>	
