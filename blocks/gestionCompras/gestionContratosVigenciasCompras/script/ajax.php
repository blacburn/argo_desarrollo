...
// Variables NUEVOOOO
$cadenaACodificarECN = "pagina=" . $this->miConfigurador->getVariableConfiguracion("pagina");
$cadenaACodificarECN .= "&procesarAjax=true";
$cadenaACodificarECN .= "&action=index.php";
$cadenaACodificarECN .= "&bloqueNombre=" . $esteBloque ["nombre"];
$cadenaACodificarECN .= "&bloqueGrupo=" . $esteBloque ["grupo"];
$cadenaACodificarECN .= "&funcion=consultaContratoEstado";
$cadenaACodificarECN .= "&usuario=" . $_REQUEST['usuario'];
$cadenaACodificarECN .="&tiempo=" . $_REQUEST['tiempo'];


// Codificar las variables
$enlace = $this->miConfigurador->getVariableConfiguracion("enlace");
$cadena = $this->miConfigurador->fabricaConexiones->crypto->codificar_url($cadenaACodificarECN, $enlace);

// URL definitiva
$urlEstadoContrato = $url . $cadena;
...
//--------------Fin JavaScript y Convenios x Vigenca --------------------------------------------------------------------------------------------------  
    ...
    $("#<?php echo $this->campoSeguro('numero_contrato') ?>").change(function () {

        if ($("#<?php echo $this->campoSeguro('numero_contrato') ?>").val() != '') {
            consultaEstado($("#<?php echo $this->campoSeguro('numero_contrato') ?>").val());
        }else
        {
            $("#<?php echo $this->campoSeguro('estado') ?>").removeAttr('disabled');
            $("#<?php echo $this->campoSeguro('clase_contrato') ?>").removeAttr('disabled');

            $("#<?php echo $this->campoSeguro('estado') ?>").select2('val', null);
            $("#<?php echo $this->campoSeguro('clase_contrato') ?>").select2('val', null);
        }
        

    });

    function consultaEstado(numero) {

        $.ajax({
            url: "<?php echo $urlEstadoContrato ?>",
            dataType: "json",
            data: {numero: numero, vigencia: $("#<?php echo $this->campoSeguro('vigencia') ?>").val(), unidad:$("#<?php echo $this->campoSeguro('unidad_ejecutora_consulta') ?>").val()},
            success: function (data) {
                if (data[0] != " ") {
                    $("#<?php echo $this->campoSeguro('estado') ?>").select2('val', data[0][1]);
                    $("#<?php echo $this->campoSeguro('estado') ?>").attr('disabled', '');

                    $("#<?php echo $this->campoSeguro('clase_contrato') ?>").select2('val', data[0][2]);
                    $("#<?php echo $this->campoSeguro('clase_contrato') ?>").attr('disabled', '');
                }
            }
