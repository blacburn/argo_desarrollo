...
//--------------Inicio JavaScript y Ajax CDP x Solicitud ---------------------------------------------------------------------------------------------    
    ...
    function ValidarExistenciaSupervisor() {
        $.ajax({
            url: "<?php echo $urlValidarSupervisor ?>",
            dataType: "json",
            data: {id: $("#<?php echo $this->campoSeguro('nuevoSupervisor') ?>").val()},
            success: function (data) {
                if (data[0] != "" && data[0] == "f") {

                    $("#<?php echo $this->campoSeguro('divisionSupervisor') ?>").css('display', 'block');

                } else {

                    $("#<?php echo $this->campoSeguro('divisionSupervisor') ?>").css('display', 'none');
                }
            }
